...
void CvHaarFeatureParams::write( FileStorage &fs ) const
{
    ...
    string modeStr = mode == BASIC ? CC_MODE_BASIC :
                     ...
                     mode == ALL ? CC_MODE_NONWAVELET :string();
...
bool CvHaarFeatureParams::read( const FileNode &node )
{
    ...
    mode = !modeStr.compare( CC_MODE_BASIC ) ? BASIC :
           ...
           !modeStr.compare( CC_MODE_ALL ) ? ALL : 
           !modeStr.compare( CC_MODE_NONWAVELET ) ? NONWAVELET :-1;
    return (mode >= 0);
}
...
void CvHaarFeatureParams::printDefaults() const
{
    ...
    cout << "  [-mode <" CC_MODE_BASIC << "(default) | "
            << CC_MODE_CORE << " | " << CC_MODE_ALL 
            << CC_MODE_NONWAVELET << endl;
}
...
void CvHaarFeatureParams::printAttrs() const
{
    ...
    string mode_str = mode == BASIC ? CC_MODE_BASIC :
                       mode == CORE ? CC_MODE_CORE :
                       mode == ALL ? CC_MODE_ALL : 
                       mode == NONWAVELET ? CC_MODE_NONWAVELET : 0;
    ...
    if ( !CvFeatureParams::scanAttr( prmName, val ) )
    {
        if( !prmName.compare("-mode") )
        {
            mode = !val.compare( CC_MODE_CORE ) ? CORE :
                   ...
                   !val.compare( CC_MODE_NONWAVELET ) ? NONWAVELET : -1;
            if (mode == -1)
                return false;
        }
        return false;
        ...
        for( int y = 0; y < winSize.height; y++ )
        {
            for( int dx = 1; dx <= winSize.width; dx++ )
            {
                for( int dy = 1; dy <= winSize.height; dy++ )
                {
                    ...
                    if (mode == CvHaarFeatureParams::NONWAVELET)
                    {
                        if (x + 4*dx <= winSize.width && y + 4*dy <= winSize.height  )
                        {
                            features.push_back( Feature( offset, false,
                                x,    y,    4*dx, 4*dy, -1,
                                x + 3*dx, y, dx, dy, +2,
                                x + dx, y + dy, dx, dy, +2, 
                                x, y + 2*dy, dx*2, dy, +2,
                                x + 3*dx, y + 2*dy, dx, dy, +2,
                                x + dx, y + 3*dy, dx, 3*dy, +2  ) );
                        }
...
CvHaarEvaluator::Feature::Feature()
{
    ...
    rect[3].r = rect[4].r = rect[5].r = rect[6].r = Rect(0,0,0,0);
    rect[0].weight = rect[1].weight = rect[2].weight = 0;
    rect[3].weight = rect[4].weight = rect[5].weight = rect[6].weight = 0;
}
...
CvHaarEvaluator::Feature::Feature( int offset, bool _tilted,
            ...
            int x6, int y6, int w6, int h6, float wt6)
{
    rect[0].r.x = x0;
    rect[0].r.y = y0;
    rect[0].r.width  = w0;
